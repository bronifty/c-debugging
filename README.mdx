...
```sh
   docker tag vsc-c-debugging-5cb14d97b6647bd269ce259609f58817373ef5391aca3ec3e9aa7ef80b9aad99 busy-knuth:latest
```
...
kubectl get pods
kubectl exec -it busy-knuth -- /bin/sh
```
...
kubectl scale deployment busy-knuth-deployment --replicas=0
kubectl get pods
kubectl scale deployment busy-knuth-deployment --replicas=1
kubectl get pods
kubectl delete pod busy-knuth-deployment-69ffb84b69-lvbm4
kubectl delete deployment busy-knuth-deployment
kubectl get deployments
...
```sh
   echo <YOUR_PAT> | docker login ghcr.io -u bronifty --password-stdin
...
```sh
docker tag busy-knuth:latest ghcr.io/bronifty/busy-knuth:latest
```

Push the image to GHCR:

```sh
docker push ghcr.io/bronifty/busy-knuth:latest
```

## Update Deployment With GHCRIO Image
...
```sh
   docker run -it ghcr.io/bronifty/busy-knuth:latest
```
...
```sh
CURRENT_DIR=$(pwd)
sed "s|/absolute/path/to/desired/directory|$CURRENT_DIR|g" overlays/dev/volume-patch.yaml > overlays/dev/volume-patch-temp.yaml
